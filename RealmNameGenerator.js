...
module.exports.getRealmName = function () {
    let rnd = Math.floor(Math.random() * names1.length);
    let rnd2 = Math.floor(Math.random() * names2.length);
    let rnd3 = Math.floor(Math.random() * names3.length);
    if (rnd > 41) {
        while (rnd3 < 12) {
            rnd3 = Math.floor(Math.random() * names3.length);
        }
    }
    let rnd4 = Math.floor(Math.random() * names4.length);
    let rnd5 = Math.floor(Math.random() * names5.length);
    return names1[rnd] + names2[rnd2] + names3[rnd3] + names4[rnd4] + names5[rnd5];
}

module.exports.getRealmDescriptiveName = function () {
    let rnd = Math.floor(Math.random() * names6.length);
    let rnd2 = Math.floor(Math.random() * names7.length);
    return "The " + names6[rnd] + " " + names7[rnd2];
}

module.exports.generateRealmNames = function (descriptive = false) {
    let names = {
        name: this.getRealmName(),
    };

    if (descriptive) {
        names.descriptiveName = this.getRealmDescriptiveName();
    }
